...
`librealsense2-framos` supports querying a set predefined attributes as part of the frame data available to the end-user.
In this explanatory we will overview
- The Design and implementation of frame metadata in `librealsense2-framos`
- The Metadata support by Intel® RealSense™ devices


## Software Design and Implementation
The library's approach on metadata is that any piece of data that provides an unique frame-related info is a potential attribute. Therefore `librealsense2-framos` does not limit itself to hardware-originated data, but establishes an infrastructure to capture and encapsulate  software-originated attributes as well.
...
When a new frame is received by Host, `librealsense2-framos` backend is responsible to process and attach the metadata attributes.  
...
## Metadata Query API
`librealsense2-framos` introduces two functions into its public API to query metadata attributes:
```cpp
/**
...
```cpp
 ...
 ```
Calling __*rs2_get_frame_metadata*__ without testing for attribute support may result in `librealsense2-framos` generating an exception. This is a design decision consistent with the error-handling model employed by librealsense2.  
...
In order for `librealsense2-framos` to get access for device-generated attributes the following system preconditions shall be met:
